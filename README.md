...
The static server is sufficient for the general planner, but accurate Quick Calc results require the `/api/quick-calc` serverless function. Configure every private deployment variable listed in `.env.example` and redeploy using a serverless environment that serves the `api` directory. When the endpoint or any required private variable is unavailable, Quick Calc fails safely and does not run an accurate fallback in the browser.
